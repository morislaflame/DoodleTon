...
import { checkBulletEnemyCollision, checkPlatformCollision, checkShootingBoostCollision, handlePlatformCollision } from '../../utils/collision';
...
const Game: React.FC = () => {
  ...
  const updateGame = useCallback((deltaTime: number) => {
    ...
    // Проверяем столкновения с бустами стрельбы
    platforms.forEach(platform => {
      if (platform.boost && 
          (platform.boost.type === 'rapidfire' || platform.boost.type === 'autofire') && 
          !platform.boost.isCollected && 
